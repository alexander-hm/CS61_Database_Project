...
mycursor.execute("SELECT fund_symbol, ROUND(AVG(volume), 0) average_volume FROM daily_performance dp INNER JOIN general g ON dp.fund_id = g.fund_id GROUP BY fund_symbol ORDER BY average_volume DESC LIMIT 10;")
result = mycursor.fetchall
...
mycursor.execute("SELECT fund_symbol, price_date, ROUND((close - open)/open, 4) AS daily_change FROM (SELECT daily_performance.*, MAX(ROUND((close - open)/open, 4)) OVER(PARTITION BY fund_id) as max_change from daily_performance) AS dp INNER JOIN general g ON dp.fund_id = g.fund_id WHERE ROUND((close - open)/open, 4) = max_change ORDER BY daily_change DESC LIMIT 10;")
result = mycursor.fetchall
